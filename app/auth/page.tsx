...
export default function AuthPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Hard navigate so server picks up the new session cookie
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ background: '#0a0f1e', border: '1px solid #0f172a', borderRadius: 20, padding: 32 }}>
          ...
          <div style={{ display: 'flex', background: '#060810', borderRadius: 12, padding: 4, marginBottom: 28 }}>
            {(['login', 'signup'] as const).map(m => (
              <button key={m} onClick={() => { setMode(m); setError('') }} style={{
                flex: 1, padding: '9px 0', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 600,
          ...
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {mode === 'signup' && (
              <div>
                <label style={{ display: 'block', fontSize: 12, color: '#64748b', marginBottom: 6, fontWeight: 600 }}>Your Name</label>
                <input style={inputStyle} placeholder="e.g. Rahul Sharma" value={name} onChange={e => setName(e.target.value)} required />
              </div>
            )}
            <div>
              <label style={{ display: 'block', fontSize: 12, color: '#64748b', marginBottom: 6, fontWeight: 600 }}>Email Address</label>
            ...
            {error && (
              <div style={{ background: '#7f1d1d22', border: '1px solid #7f1d1d', borderRadius: 8, padding: '10px 14px', color: '#fca5a5', fontSize: 13 }}>
                {error}
              </div>
            )}
            ...
            <button type="submit" disabled={loading} style={{
              ...
              boxShadow: loading ? 'none' : '0 4px 20px rgba(220,38,38,0.35)', transition: 'all 0.2s', marginTop: 4
            }}>
        ...
        <p style={{ textAlign: 'center', color: '#334155', fontSize: 13, marginTop: 24 }}>
          Share this link with family members so they can create their own accounts.
        </p>
      </div>
    </div>
  )
